...
from .models import Topic, Entry #models前面的句点让Django在当前目录中寻找models.py
...
def new_topic(request):
    ...
    if request.method != 'POST' :
        """Two main types of request: GET && POST, GET means request the empty form"""
        form = TopicForm()
    else:
...
def new_entry(request, topic_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            new_entry = form.save(commit=False) # 不保存在数据库中
